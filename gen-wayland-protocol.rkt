...
(define (Interface-dump i client-test-out server-test-out)
  ...
  (define client-filename (interface-name->client-filename name))
  (define server-filename (interface-name->server-filename name))
  (define client-path (interface-name->client-path name))
  (define server-path (interface-name->server-path name))
  (pretty-write `(require ,client-filename) client-test-out)
  (pretty-write `(require ,server-filename) server-test-out)
...
(require \"libwayland-server.rkt\" \"../wayland-util.rkt\" \"../wayland-private.rkt\" \"../wayland-server.rkt\")
...
(require ffi/unsafe ffi/unsafe/define)
(require \"libwayland-client.rkt\" \"../wayland-util.rkt\" \"../wayland-private.rkt\" \"../wayland-client.rkt\")
...
(define (interface-name->client-path n)
  (format "generated/~a" (interface-name->client-filename n)))

(define (interface-name->server-filename n)
  (format "~a-server.rkt" n))

(define (interface-name->server-path n)
  (format "generated/~a" (interface-name->server-filename n)))
...
(call-with-output-file
  "generated/client-test.rkt"
  (lambda (client-test-out)
    (call-with-output-file
      "generated/server-test.rkt"
...
(call-with-output-file
  "generated/libwayland-client.rkt"
  (lambda (out) (display client-lib-header out))
  #:mode 'text #:exists 'replace)

(define server-lib-header
  (format "\
#lang racket/base
